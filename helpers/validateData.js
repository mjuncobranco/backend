...
const validateData = (params) => {
  ...
  if (params.name) {
    if (!validator.isLength(params.name, { min: 3, max: 50 }) || !validator.isAlpha(params.name)) {
  ...
  if (params.surname) {
    if (!validator.isLength(params.surname, { min: 4, max: 50 }) || !validator.isAlpha(params.surname)) {
  ...
  if (params.nick) {
    if (!validator.isLength(params.nick, { min: 3, max: 10 })) {
      throw new Error("Invalid nick: it should be between 3-10 characters.");
    }
  }
